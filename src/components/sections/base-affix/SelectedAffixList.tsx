...
// the step label, and the mode-aware subtitle ("all must be present" /
// "items matching any of these"). This component renders only the chip
// list itself (or an inline placeholder for empty/loading/error states).
...
export function SelectedAffixList({
  ...
  if (error) {
    return <p className="text-sm text-red-400">Failed to load affixes: {error.message}</p>;
  }

  if (selectedAffixes.length === 0) {
    return <p className="text-sm text-gray-500 italic">No affixes selected yet</p>;
  }
          ...
          >
            ...
            {isEditing && !isLoading ? (
              <span className="flex items-center gap-1">
                ...
                <span className="text-gray-100 min-w-[2ch] text-center">{tierLabel(selected)}</span>
                <button
                  type="button"
                  onClick={() => handleBumpTier(index, selected, affix, 1)}
