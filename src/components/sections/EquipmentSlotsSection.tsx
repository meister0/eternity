import { EQUIPMENT_SLOT_MACROS } from '../../data/stash-macros';
import type { EquipmentSlotsSectionProps } from '../types/component-props';
import { SectionContainer, SectionHeader } from '../ui';

export function EquipmentSlotsSection({
  equipmentSlots,
  toggleSetItem,
}: EquipmentSlotsSectionProps) {
  return (
    <SectionContainer className="mb-6 md:mb-8">
      <SectionHeader>Equipment Slots</SectionHeader>
      <div className="space-y-4">
        {EQUIPMENT_SLOT_MACROS.map((group) => (
          <div key={group.label}>
            <label className="block text-xs font-medium text-gray-400 mb-2">{group.label}</label>
            <div className="flex flex-wrap gap-2">
              {group.items.map((item) => {
                const isActive = equipmentSlots.has(item.value);
                return (
                  <button
                    key={item.value}
                    onClick={() => toggleSetItem('equipmentSlots', item.value)}
                    className={`px-3 py-2.5 md:py-2 rounded text-xs md:text-sm font-medium transition-colors cursor-pointer ${
                      isActive
                        ? 'bg-amber-600 text-white hover:bg-amber-700'
                        : 'bg-gray-600 text-white hover:bg-gray-700 border border-gray-600'
                    }`}
                  >
                    {item.label}
                  </button>
                );
              })}
            </div>
          </div>
        ))}
      </div>
    </SectionContainer>
  );
}
